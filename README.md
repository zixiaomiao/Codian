...
当前版本已完成到 **第二阶段：Memory 分类体系**。

已完成：

- 基础长期记忆读写：读取、追加、精简检索 `Codex/Codex 会话总结.md`。
- 第一阶段 Project Summary：生成并优先读取 `Codex/project-summary.md`。
- 第二阶段 Memory 分类体系：生成 `Codex/memory/Project.md`、`Decision.md`、`Todo.md`、`Bug.md`、`User-Preference.md`。
- 按关键词读取匹配分类，再读取启动规则和少量命中日志。
- 选择性同步 GitHub：只同步 Codex 记忆文件，不管理整个 Obsidian vault。
- 已验证本地源码、Codex 已安装插件缓存和 GitHub `main` 功能一致。

未完成：

- 第三阶段 Decision Log：独立记录“为什么这样做”的决策说明。
- 第四阶段自动摘要与归档：压缩旧日志，避免记忆库无限膨胀。
- 第五阶段记忆质量优化：重复检测、相似合并、重要性评分。
- 第六阶段 Generate Context：一键生成适合 Codex、Claude Code、Cursor 等工具读取的项目上下文。

## 功能

- 从你的 Obsidian vault 读取 `Codex/Codex 会话总结.md`。
- 从会话总结生成 `Codex/project-summary.md` 项目摘要。
- 从会话总结生成 `Codex/memory/` 下的分类记忆文件。
- 初始化时可自动创建一份通用“启动必读”记忆模板。
- 默认优先读取项目摘要；带关键词时可读取匹配分类，再读取启动规则、读取策略、检索索引和固定路径，避免每次展开完整历史。
...
- 分类任务：按关键词读取 `Codex/memory/` 中的匹配分类。
...
第二阶段加入 Project Summary 和 Memory 分类后，常规读取路径更稳定：先读项目摘要，再按关键词读取分类文件，最后才带出少量历史日志。这样可把“长期记忆上下文”稳定控制在几千 token 以内，同时让稳定内容更容易命中缓存。

| 场景 | 读取内容 | 估算 token | 相比完整读取 |
| --- | --- | --- | --- |
| 不读记忆 | 无长期记忆 | 0 | 最省，但没有记忆收益 |
| 插件默认读取 | Project Summary、启动必读、读取策略、索引、固定路径 | 约 1k-2k | 约省 80%-90% |
| 插件 + 分类读取 | 默认读取 + 1 个匹配分类文件 | 约 1.5k-3k | 约省 70%-90% |
| 插件 + 分类 + 关键词检索 | 默认读取 + 匹配分类 + 1-3 条相关历史 | 约 2k-4k | 约省 65%-85% |
| 完整读取会话总结 | 整篇记忆文件 | 约 9k-16k | 基准 |

缓存命中方面，Project Summary、启动必读和分类文件比完整日志更稳定，通常更容易复用缓存；关键词命中的历史日志会随任务变化，缓存命中率会中等一些；完整读取虽然也有稳定部分，但日志不断追加，体量和顺序变化更容易降低实际收益。
